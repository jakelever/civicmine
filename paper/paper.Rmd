...
Precision oncology involves analysis of individual cancer samples to understand the genes and pathways involved in the development and progression of a cancer. To improve patient care, knowledge of diagnostic, prognostic, predisposing and drug response markers is essential. Several knowledgebases have been created by different groups to collate current knowledge on these efforts including the open-access Clinical Interpretations of Variants in Cancer (CIViC) knowledgebase. These databases rely on time-consuming manual curation from highly-skilled experts who read and interpret the relevant biomedical literature. To aid in this curation and provide the greatest coverage for these databases, particularly CIViC, we propose the use of text mining approaches to extract these clinically relevant markers from all available published literature. A group of cancer genomics experts annotated biomarkers discussed in 800 sentences and achieved good inter-annotator agreement. We used a supervised learning approach to construct the CIViCmine knowledgebase and extracted `r paper.sentenceCount` relevant sentences from PubMed abstracts and Pubmed Central Open Access full text papers.  CIViCmine contains over `r paper.biomarkerCount` biomarkers associated with `r paper.geneCount` genes, `r paper.drugCount` drugs and `r paper.cancerCount` cancer types, representing `r paper.abstractCount` abstracts and `r paper.articleCount` full-text publications. Through integration with CIVIC, we provide a prioritised list of curatable biomarkers as well as a resource that is valuable to other knowledgebases and precision cancer analysts in general.

## Author Summary {-}

The future of cancer treatment will involve examining the DNA and other molecular biomarkers of individual patient’s tumours. This will allow for a personalized treatment strategy to be designed for each patient. Current approaches for identifying biomarker-guided treatments require researchers to read vast amounts of scientific literature about cancer mutations. These papers discuss how molecular data can help diagnose a cancer, forecast its aggressiveness, understand the risk of developing cancer and predict whether a drug will be effective. The CIViC database is an online resource collating this information into a central repository. However there are too many papers published every month on this topic to allow for constant manual review and update. To help the curators of CIViC know which papers are relevant, we have built a text mining system that extracted and prioritized `r paper.sentenceCount` sentences discussing cancer mutations. This resource is valuable to the CIViC curators and other researchers working towards personalized cancer treatment.
...
knitr::kable(
  traintest,
  booktabs = TRUE, escape=FALSE,
  caption = 'Number of annotations in the 75%/25% training and test split',
...
```{r examplesentences, echo=F, eval=T, fig.width=8, fig.asp=0.7,  fig.cap='(ref:examplesentences)'}
...
The CIViCmine resource was evaluated by a CIViC curator. They selected the top 100 biomarkers of each evidence item deemed to not already be in CIViC. They rated each one based on its appropriateness for CIViC, whether the text extraction was correct and whether it should be curated in CIViC. Table X shows that…
...
CIViC is supported by the National Cancer Institute (NCI) of the National Institutes of Health (NIH) under award number U01CA209936 to O.L.G. (with M.G. and E.R.M. as co-investigators). J.L. was supported by a Vanier Canada Graduate Scholarship. M.G. was supported by the NHGRI under award number R00HG007940. O.L.G. was supported by the NCI under award number K22CA188163. The authors would like to thank Compute Canada for the computational infrastructure used.
